...
#endif
#ifdef GMRES
    AsyncILU precond = AsyncILU(20, 1e-4);
    std::unique_ptr<FastGMRES> solver;
#endif
#ifdef SpLU
...
#ifdef GMRES
        Eigen::VectorXd phi_next = solver->solve(A, b, precond, 1000, 1e-8);
#endif
...
#endif 
#ifdef GMRES
        precond.compute(A, omp_get_max_threads()); 
        solver = std::make_unique<FastGMRES>(A.rows(), 50);
#endif
...
#ifdef GMRES
        Eigen::VectorXd phi_np1 = solver->solve(A, b, precond, 1000, 1e-8);
#endif
